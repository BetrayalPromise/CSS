...
static void aspect_hookedGetClass(Class class, Class statedClass) {
    ...
    IMP newIMP = imp_implementationWithBlock(^(id self) { return statedClass; });
...
static void _aspect_modifySwizzledClasses(void (^block)(NSMutableSet *swizzledClasses)) {
    ...
    dispatch_once(&pred, ^{ swizzledClasses = [NSMutableSet new]; });
...
static NSMutableDictionary *aspect_getSwizzledClassesDict() {
    ...
    static dispatch_once_t pred;
    dispatch_once(&pred, ^{ swizzledClassesDict = [NSMutableDictionary new]; });
...
static BOOL aspect_isSelectorAllowedAndTrack(NSObject *self, SEL selector, AspectOptions options, NSError **error) {
    ...
    static dispatch_once_t pred;
    dispatch_once(&pred, ^{ disallowedSelectorList = [NSSet setWithObjects:@"retain", @"release", @"autorelease", @"forwardInvocation:", nil]; });

    // Check against the blacklist.
    /// 如果是retain release autorelease forwardInvocation: 这几个selector是不支持切面的
...
- (BOOL)removeAspect:(id)aspect {
    for (NSString *aspectArrayName in @[ NSStringFromSelector(@selector(beforeAspects)), NSStringFromSelector(@selector(insteadAspects)), NSStringFromSelector(@selector(afterAspects)) ]) {
