...
- (NSString *)initials {
    ...
    return [[components take:3] reduce:^id(NSString *carry, NSString *component) { return [carry stringByAppendingString:[component substringToIndex:1]]; } carry:@""];
}
...
- (NSString *)substr:(int)from {
    if (from >= 0)
        return [self substringFromIndex:MIN(from, (int) self.length)];
    else
        return [self substringFromIndex:MAX((int) self.length + from, 0)];
}
...
- (NSString *)withoutDiacritic {
    return [self stringByFoldingWithOptions:NSDiacriticInsensitiveSearch locale:[NSLocale systemLocale]];
}
...
- (NSString *)lpad:(int)lenght string:(NSString *)string {
    ...
    NSString *padChars = [[NSString string] stringByPaddingToLength:finalLength withString:string startingAtIndex:0];

    return [padChars append:self];
}

- (NSString *)rpad:(int)lenght string:(NSString *)string {
    return [self stringByPaddingToLength:MAX((int) self.length, lenght) withString:string startingAtIndex:0];
}
