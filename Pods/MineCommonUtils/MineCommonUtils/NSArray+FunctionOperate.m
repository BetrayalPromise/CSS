...
@implementation NSArray (FunctionOperate)

- (NSArray<id> *_Nullable)filter:(BOOL (^_Nonnull)(id _Nonnull object))condition {
    if (condition) {
        return [self filteredArrayUsingPredicate:[NSPredicate predicateWithBlock:^BOOL(id _Nonnull evaluatedObject, NSDictionary<NSString *, id> *_Nullable bindings) { return condition(evaluatedObject); }]];
    }
    NSLog(@"过滤条件为nil");
    return nil;
}
...
- (_Nullable id)first:(BOOL (^_Nonnull)(id _Nonnull object))condition __attribute__((nonnull(1))) {
    if (condition) {
        NSUInteger index = [self indexOfObjectPassingTest:^BOOL(id _Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) { return condition(obj); }];
...
- (void)each:(void (^_Nullable)(id _Nonnull object))operation {
    if (operation) {
        [self enumerateObjectsUsingBlock:^(id object, NSUInteger idx, BOOL *stop) { operation(object); }];
    }
}

- (void)eachWithIndex:(void (^_Nullable)(id _Nonnull object, int index, BOOL *_Nonnull stop))operation {
    if (operation) {
        [self enumerateObjectsUsingBlock:^(id object, NSUInteger idx, BOOL *stop) { operation(object, (int) idx, stop); }];
    }
}

- (NSArray<id> *_Nullable)sort {
    return [self sortedArrayUsingComparator:^NSComparisonResult(id a, id b) { return [a compare:b]; }];
}
...
- (NSArray<id> *_Nullable)sortWith:(NSComparisonResult (^_Nullable)(id _Nonnull a, id _Nonnull b))callback {
    if (callback) {
        return [self sortedArrayUsingComparator:^NSComparisonResult(id a, id b) { return callback(a, b); }];
    }
    return self;
}

- (NSArray<id> *_Nullable)sortWithNilAtTheEnd:(NSString *_Nullable)key ascending:(BOOL)ascending {
    NSSortDescriptor *sortDescriptor = [NSSortDescriptor sortDescriptorWithKey:key ascending:!ascending comparator:^NSComparisonResult(id obj1, id obj2) { return [obj2 compare:obj1]; }];
    return [self sortedArrayUsingDescriptors:@[ sortDescriptor ]];
}
...
- (NSArray<id> *_Nullable)flatMap:(id _Nonnull (^_Nullable)(id _Nonnull obj, NSUInteger idx))block {
    if (block) {
        ...
        [self each:^(NSArray *array) { [results addObject:[array map:^id(id obj, NSUInteger idx) { return block(obj, idx); }]]; }];
        return results;
    }
    return nil;
}
...
- (NSArray<id> *_Nullable)flatMap:(NSString *_Nullable)keyPath block:(_Nonnull id (^_Nullable)(id _Nonnull obj, NSUInteger idx))block {
    if (keyPath && block) {
        NSMutableArray *results = [NSMutableArray new];
        [self each:^(id object) { [results addObject:[[object valueForKeyPath:keyPath] map:^id(id obj, NSUInteger idx) { return block(obj, idx); }]]; }];
        return results;
    }
    return nil;
}
...
- (NSArray<id> *_Nullable)flatten {
    NSMutableArray *results = [NSMutableArray new];
    [self each:^(NSArray *array) { [results addObjectsFromArray:array]; }];
    return results;
}
...
- (NSArray<id> *_Nullable)flatten:(NSString *_Nullable)keyPath {
    if (keyPath) {
        NSMutableArray *results = [NSMutableArray new];
        [self each:^(id object) { [results addObjectsFromArray:[object valueForKeyPath:keyPath]]; }];
        return results;
    }
    return nil;
}
...
- (NSDictionary *_Nullable)pluck:(NSString *_Nullable)keyPath key:(NSString *_Nullable)keyKeyPath {
    if (keyPath && keyKeyPath) {
        ...
        [self each:^(id object) { result[[object valueForKey:keyKeyPath]] = [object valueForKey:keyPath]; }];
        return result;
    }
    return nil;
}
...
- (id _Nullable)reduce:(id _Nonnull (^_Nullable)(id _Nonnull carry, id _Nonnull object))block carry:(id _Nullable)carry {
    if (block) {
        ...
        [self each:^(id object) { carry2 = block(carry2, object); }];
        return carry2;
    }
    return nil;
}
...
- (NSDictionary *_Nullable)groupBy:(NSString *_Nullable)keyPath {
    if (keyPath) {
        return [self groupBy:keyPath block:^NSString *(id object, NSString *key) { return key; }];
    }
    return nil;
}
...
- (NSDictionary *_Nullable)expand:(NSString *_Nullable)keypath unique:(BOOL)unique {
    if (keypath) {
        if (unique)
            keypath = [NSString stringWithFormat:@"%@.@distinctUnionOfObjects.self", keypath];
        NSMutableDictionary *result = [NSMutableDictionary new];
        [self each:^(id object) {
            [[object valueForKeyPath:keypath] each:^(id key) {
                if (result[key] == nil)
                    result[key] = [NSMutableArray new];
...
- (id _Nullable)maxObject {
    if (self.count > 0) {
        return [self reduce:^id(id carry, id object) { return (object > carry) ? object : carry; } carry:self.firstObject];
    }
    return nil;
}
...
- (id _Nullable)maxObject:(NSString *_Nullable)keyPath {
    if (keyPath) {
        return [self reduce:^id(id carry, id object) { return ([[object valueForKeyPath:keyPath] doubleValue] > [[carry valueForKeyPath:keyPath] doubleValue]) ? object : carry; } carry:self.firstObject];
    }
    return nil;
}
...
- (id _Nullable)maxObjectFor:(double (^_Nullable)(id _Nonnull obj))block {
    if (block) {
        return [self reduce:^id(id carry, id object) { return block(object) > block(carry) ? object : carry; } carry:self.firstObject];
    }
    return nil;
}
...
- (NSNumber *) operator:(NSString *) operator keyPath:(NSString *)keyPath {
    ...
    if (keyPath != nil)
        ...
        finalKeyPath = [NSString stringWithFormat:@"@%@.self", keyPath];
...
- (NSNumber *_Nullable)sumWith:(NSNumber *_Nonnull (^_Nullable)(id _Nonnull object))block {
    if (block) {
        return [self reduce:^id(NSNumber *carry, id object) { return @(carry.floatValue + block(object).floatValue); } carry:@(0)];
    }
    return [self sum];
}
